...
export function AdoptBaDialog({
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    fetch("/api/ba-templates?type=substance")
      .then((res) => res.json())
  ...
  const handleAdopt = async (documentId: string) => {
    ...
    try {
      const res = await fetch(`/api/substances/${substanceId}/adopt-ba`, {
        method: "POST",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Wählen Sie eine vorhandene Betriebsanweisung für &quot;{substanceName}&quot;.
